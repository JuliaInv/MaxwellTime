...
function getSensTMatVec(z::Array{Float64,1},sigma::Array{Float64,1},
	...
	for i=length(dt)-1:-1:1
	    ...
	    if norm(rhs) > 1e-20
	      lam[:,j,1],mySolver = solveLinearSystem(A,rhs,mySolver,1,0)
	    end	
...
function getSensTMatVec(z::Array{Float64,1},sigma::Array{Float64,1},
	...
	for i=nt:-1:2
	    ...
	    if norm(rhs) > 1e-20
	      lam[:,j,1],EMsolver = solveLinearSystem(A,rhs,EMsolver,1,0)
	      EMsolver.doClear = 0
	    end	
...
# 	clear!(mySolver)
	...
	for j = 1:ns
	  lmTmp[:,j],EMsolver = solveLinearSystem(A,3/(4*dt)*Msig*lam[:,j,2],EMsolver,1,0)
	...
	if groundedSource
	  ...
	  for j = 1:ns
	    ...
	    if norm(rhs) > 1e-20
	      lam0,DCsolver = solveLinearSystem(A0,rhs,DCsolver,1,0)
	    end
