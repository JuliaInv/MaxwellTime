using MaxwellTime
using JOcTree, Elliptic
using PyPlot
using Base.Test
import CGI.MaxwellTime
import jInv.Mesh: AbstractMesh
...
AQloop = ReducedMeshVectorPotential(M,rModel,[h/2;h/2;txHeight])
#Adiff = norm(Aloop-AQloop)/norm(AQloop)
...
pForQloop = getMaxwellTimeParam(M,sQloop,P,obsTimes,t0,dt,wave,sourceType;
                                 timeIntegrationMethod=:BDF2Const,
                                 verbosity=2)
...
println("Getting Q averaged A analytic loop source data")
dLoop,pForLoop = getData(sigma,pForLoop)
println("Getting Qloop's analytic loop source data")
dQloop,pForQloop = getData(sigma,pForQloop)
# dLoop = -dLoop
...
loglog(obsTimes[p2],dLoop[p2],"r-",label="Analytic loop averaged")
#loglog(obsTimes[m2],abs.(dLoop[m2]),"r--")
loglog(obsTimes[p3],dbzdt[p3],"m-",label="true solution")
#loglog(obsTimes[m3],abs.(dbzdt[m3]),"m--")
loglog(obsTimes[p4],dQloop[p4],"g-",label="Analytic loop reduced mesh A")
#loglog(obsTimes[m4],abs.(dQloop[m4]),"g--")
...
err3 = abs.(dQloop-dbzdt)./dbzdt;
figure()
loglog(obsTimes,err1,"b-",label="Square wire path BDF2")
loglog(obsTimes,err2,"r-",label="Analytic loop BDF2 averaged")
loglog(obsTimes,err3,"g-",label="Analytic loop reduced mesh A")
legend()
